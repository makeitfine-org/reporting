...
This is a **production-grade Spring Boot 3.x microservice** responsible for banking financial reporting. It implements a
**CQRS (Command Query Responsibility Segregation) read model** pattern, consuming events from a legacy monolith via
Kafka to build high-performance query projections in Elasticsearch.
...
* **Infrastructure Layer (`infrastructure`):** Implementations for persistence (Postgres, ES), messaging (Kafka), and
  caching (Redis).
...
3. **Query:** API clients request reports; data is fetched from Redis (cache hit) or aggregated from Elasticsearch (
   cache miss).
...
* **Git:** Use semantic commit messages (e.g., `feat: add revenue report endpoint`,
  `fix: correct kafka consumer group id`).
